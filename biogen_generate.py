...
from utils.constants import (
    BIOGEN_TEMPLATE_WITH_CONTEXT,
    BIOGEN_TEMPLATE_WITH_CONTEXT_PREFIX,
    BIOGEN_TEMPLATE_WITHOUT_CONTEXT,
...
for id, sample in tqdm(dataset.items()):
    ...
    if args.without_contexts:
        prompt = BIOGEN_TEMPLATE_WITHOUT_CONTEXT.format(
            question=question,
        )
        prompt_prefix = BIOGEN_TEMPLATE_WITHOUT_CONTEXT_PREFIX.format(
            question=question,
        )
        run_name = f"{args.model_name_or_path.replace('/', '_')}_{args.method}_without_contexts"
    else:
        prompt = BIOGEN_TEMPLATE_WITH_CONTEXT.format(
            question=question,
            contexts="\n\n".join(contexts),
        )
        prompt_prefix = BIOGEN_TEMPLATE_WITH_CONTEXT_PREFIX.format(
            question=question,
            contexts="\n\n".join(contexts),
        )
    ...
    json.dump(results, open(f"biogen/{run_name}.json", "w"), indent=4)
