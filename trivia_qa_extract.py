...
database = {
    "metadata": {
        "dataset": "trivia_qa/rc.wikipedia",
        "split": "validation",
...
for id, sample in enumerate(tqdm(dataset)):
    ...
    docs = []
    for doc in sample["entity_pages"]["wiki_context"]:
    ...
    database[id] = {
        "question": question,
        "answers": sample["answer"]["aliases"] + [sample["answer"]["value"]] + [sample["answer"]["normalized_value"]],
...
json.dump(database, open("trivia_qa/rc_wikipedia_validation.json", "w"), indent=2)
